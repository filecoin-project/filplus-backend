...
use crate::models::allocation_amounts::{ActiveModel, Column, Entity as AllocationAmount, Model as AllocationAmountModel, QuantityOptionModel};
 ...
 */
pub async fn get_allocation_quantity_options(allocator_id: i32) -> Result<Vec<String>, sea_orm::DbErr> {
    let conn = get_database_connection().await?;
 ...
 */
pub async fn create_allocation_amount(allocator_id: i32, allocation_amount: String) -> Result<(), sea_orm::DbErr> {
    let conn = get_database_connection().await?;
...
* @return Result<(), sea_orm::DbErr> - The result of the operation
*/
pub async fn delete_allocation_amounts_by_allocator_id(allocator_id: i32) -> Result<(), sea_orm::DbErr> {
   let conn = get_database_connection().await?;
