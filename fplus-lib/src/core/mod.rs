...
use self::application::file::{
    AllocationRequest, AllocationRequestType, AppState, ApplicationDatacap, ApplicationFile,
    NotaryInput, ValidNotaryList, ValidRKHList,
};
use rayon::prelude::*;
...
impl LDNApplication {
    ...
    pub async fn new_from_issue(info: CreateApplicationInfo) -> Result<Self, LDNError> {
        ...
        let application_datacap = ApplicationDatacap::from(parsed_ldn.datacap);
